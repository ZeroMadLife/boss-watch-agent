...
describe("BossHunter Browser Runtime HTTP client", () => {
  it("uses the documented health, targets, and fixed-evaluation endpoints", async () => {
    ...
    await expect(client.waitForLoad("target-opened-1", 100)).resolves.toBeUndefined();
  ...
  it("does not poll a page load after the caller has cancelled", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch");
    const abortController = new AbortController();
    abortController.abort();
    const client = new HttpBossHunterBrowserRuntime("http://127.0.0.1:3456");

    await expect(
      client.waitForLoad("target-cancelled", 15_000, abortController.signal),
