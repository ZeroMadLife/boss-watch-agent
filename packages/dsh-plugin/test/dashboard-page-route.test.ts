...
test('serves a user-facing personal job board without interpolating job facts', () => {
  ...
  assert.match(visibleMarkup, /简历中心/u)
  assert.match(visibleMarkup, /id="resume-list"/u)
  assert.match(visibleMarkup, /导入新版本/u)
  assert.match(visibleMarkup, /完善预填资料/u)
  assert.match(response.body(), /\/boss-watch\/api\/v1\/dashboard/u)
  assert.match(visibleMarkup, /id="match-filter"/u)
  assert.match(visibleMarkup, /id="application-filter"/u)
  ...
  assert.match(response.body(), /飞书待同步/u)
  assert.match(response.body(), /飞书已记录/u)
  assert.match(response.body(), /本地确认/u)
  ...
  assert.doesNotMatch(response.body(), /resume\.displayName|resume\.localArtifactRef|resume\.contentHash|resume\.text/u)
})

test('renders a privacy-bounded resume center and draft-only import controls', () => {
  const response = captureResponse()
  handleBossWatchDashboardPageRequest({ method: 'GET' }, response.value)

  assert.match(response.body(), /function renderResumeCenter\(\)/u)
  assert.match(response.body(), /version\.current \? '当前简历' : '历史版本 '/u)
  assert.match(response.body(), /已解析用于匹配/u)
  assert.match(response.body(), /待首次匹配解析/u)
  assert.match(response.body(), /version\.matchedJobCount/u)
  assert.match(response.body(), /buildResumeDraft\(kind\)/u)
  assert.match(response.body(), /不要回显手机号、邮箱、证件号或简历正文/u)
  assert.match(response.body(), /delivery: 'draft_only', autoSubmit: false/u)
  assert.doesNotMatch(response.body(), /删除简历|编辑简历正文/u)
})

test('keeps confirmed local facts and Feishu projection filters distinct', () => {
  const response = captureResponse()
  handleBossWatchDashboardPageRequest({ method: 'GET' }, response.value)

  assert.match(response.body(), /<option value="not_applied">待投递<\/option>/u)
  assert.match(response.body(), /<option value="submitted">已投递<\/option>/u)
  assert.match(response.body(), /<option value="feishu_pending">飞书待同步<\/option>/u)
  assert.match(response.body(), /<option value="feishu_recorded">飞书已记录<\/option>/u)
  assert.match(response.body(), /function applicationState\(candidate\)/u)
  assert.match(response.body(), /candidate\.nextAction === 'sync_feishu'/u)
  assert.match(response.body(), /飞书未配置/u)
  assert.doesNotMatch(response.body(), /实时同步|双向同步/u)
})
