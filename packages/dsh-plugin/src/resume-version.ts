...
export class LocalResumeImportService {
  ...
  async preview(input: ResumeImportInput): Promise<ResumeImportPreview> {
    ...
    return {
      ...
      ...prepared.supersedesResumeVersionId === undefined ? {} : { supersedesResumeVersionId: prepared.supersedesResumeVersionId },
  ...
  async #prepare(input: ResumeImportInput): Promise<PreparedResume> {
    ...
    const supersedesResumeVersionId = input.supersedesResumeVersionId === undefined
      ? undefined
...
function isCommandUnavailable(error: unknown): boolean {
  return error instanceof Error && 'code' in error && (error.code === 'ENOENT' || error.message === 'resume_text_extraction_unavailable')
}
...
function fromRow(row: ResumeVersionRow): ResumeVersion {
  return {
    ...
    ...row.supersedes_resume_version_id === null ? {} : { supersedesResumeVersionId: row.supersedes_resume_version_id },
  }
}
...
function validateResumeVersion(version: ResumeVersion): void {
  ...
  if (!Number.isSafeInteger(version.byteSize) || version.byteSize < 1 || version.byteSize > MAX_RESUME_BYTES) {
  ...
  if (version.supersedesResumeVersionId !== undefined) normalizeResumeVersionId(version.supersedesResumeVersionId)
}

function normalizeFileName(value: string): string {
  const normalized = value.trim()
  if (normalized.length === 0 || normalized.length > 255 || normalized !== basename(normalized) || /[\u0000-\u001f\u007f]/u.test(normalized)) {
