...
import type { RecruitmentSourceService, RecruitmentSourceStore } from "./recruitment-source.js";
...
const FOLLOW_UP = {
  ...
  properties: {
    ...
    reason: {
      type: "string",
      required: true,
      enum: ["application_status", "no_response", "interview", "manual"],
    },
    note: { type: "string" },
...
const RECRUITMENT_SOURCE = {
  ...
  properties: {
    ...
    sourceType: {
      type: "string",
      required: true,
      enum: ["official_referral", "company_career", "campus_announcement", "unknown"],
    },
...
const CANDIDATE_BOARD_ITEM = {
  ...
  properties: {
    ...
    recordKind: {
      type: "string",
      required: true,
      enum: ["recruitment_source", "source_lead", "captured_job"],
    },
    sourceKind: { type: "string", required: true },
    company: { type: "string", required: true },
    role: { type: "string", required: true },
...
const INTERVIEW_NOTE_PREVIEW = {
  ...
  properties: {
    ...
    stage: {
      type: "string",
      required: true,
      enum: ["screening", "first_interview", "second_interview", "final_interview", "other"],
    },
    contentHash: { type: "string", required: true },
...
const WATCH = {
  ...
  properties: {
    ...
    state: {
      type: "string",
      required: true,
      enum: ["active", "polling", "paused_human_required", "stopped"],
    },
    createdAt: { type: "string", required: true },
    ...
    lastResult: {
      type: "string",
      enum: ["unchanged", "changed", "transient_failure", "paused_human_required"],
    },
    pausedReason: { type: "string" },
  },
} as const;
...
const CANDIDATE_PROFILE_STATUS = {
  type: "string",
  required: true,
  enum: ["ok", "source_unavailable", "not_found", "invalid_request", "conflict"],
} as const;
  ...
  const disposers = [
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Preview a user-pasted company recruitment link and referral code. Local parsing only; does not open the link, create a JobLead, match a resume, or write Feishu.",
        parameters: {
          rawText: {
            type: "string",
            required: true,
            description: "Untrusted pasted source text containing a company and HTTPS recruitment link.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          if (recruitmentSource === undefined)
            return { status: "source_unavailable" as const, message: "recruitment_source_unavailable" };
          try {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Preview binding one confirmed recruitment source to an exact role and complete official JD. Local-only: validates and hashes evidence but does not write, browse, match a resume, or submit an application.",
        parameters: {
          sourceId: { type: "string", required: true },
          role: { type: "string", required: true },
          officialJobUrl: {
            type: "string",
            required: true,
            description: "Exact public HTTPS job or application URL selected by the user.",
          },
          jdText: {
            type: "string",
            required: true,
            description:
              "Complete visible JD text supplied by the user. It is hashed in preview and never returned.",
          },
          city: { type: "string" },
          cohort: { type: "string" },
          recruitmentType: { type: "string" },
        ...
        async execute(args) {
          if (recruitmentJd === undefined)
            return { status: "source_unavailable" as const, message: "recruitment_jd_service_unavailable" };
          try {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "After explicit confirmation, persist the previewed official JD, create a verified company-career JobLead, and bind both identifiers back to the recruitment source. Does not browse, match, fill, or submit.",
        parameters: {
          previewToken: { type: "string", required: true },
          confirmation: {
            type: "string",
            required: true,
            description:
              "Explicit confirmation of source, company, role, exact URL, JD hash and optional metadata shown in preview.",
          },
        },
        output: {
          schema: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Apply exactly one previously previewed recruitment source after explicit confirmation. Writes only the local source inbox; it does not invent a role/JD or visit the official site.",
        parameters: {
          previewToken: {
            type: "string",
            required: true,
            description: "Short-lived token returned by boss_watch_recruitment_source_preview.",
          },
          confirmation: {
            type: "string",
            required: true,
            description:
              "Explicit confirmation of the displayed company, HTTPS link, referral code and hash.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          ...
          if (typeof args.confirmation !== "string" || args.confirmation.trim().length === 0) {
            return {
              ...
              message: "recruitment_source_confirmation_required",
            };
          }
          try {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "List locally confirmed recruitment sources. Read-only; does not contact external sites or infer roles/JDs.",
        parameters: {
        ...
        async execute(args) {
          if (recruitmentSourceStore === undefined)
            return {
              status: "source_unavailable" as const,
              sources: [],
              count: 0,
              message: "recruitment_source_store_unavailable",
            };
          try {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Read one confirmed recruitment source by id. Read-only; role and JD remain absent until explicitly bound later.",
        parameters: {
          sourceId: { type: "string", required: true },
        },
        output: {
        ...
        async execute(args) {
          if (recruitmentSourceStore === undefined)
            return { status: "source_unavailable" as const, message: "recruitment_source_store_unavailable" };
          try {
    ...
    ctx.tools.register(
      defineTool({
        name: "boss_watch_candidate_profile_get",
        description:
          "Read which reusable local ATS preference fields are configured. Returns only field names, hash and update time, never stored values.",
        parameters: {},
        output: {
          schema: {
            type: "object",
        ...
        async execute() {
          if (candidateProfile === undefined)
            return { status: "source_unavailable" as const, message: "candidate_profile_unavailable" };
          try {
            const profile = candidateProfile.getSummary();
            return profile === undefined
              ? { status: "not_found" as const, message: "candidate_profile_not_configured" }
              : { status: "ok" as const, profile: profile as unknown as JsonValue };
          } catch (error: unknown) {
            return candidateProfileError(error);
          }
        },
      }),
    ),
    ctx.tools.register(
      defineTool({
        name: "boss_watch_candidate_profile_preview",
        description:
          "Preview a partial update to reusable local ATS preferences. Values are user-provided and remain in local SQLite; the result returns only availability and hashes. Does not inspect or fill a page.",
        parameters: {
          preferredCity: {
            type: "string",
            description: "Reusable preferred city when a JobLead has no exact city.",
          },
          arrivalTime: { type: "string", description: "Reusable availability or arrival-time answer." },
          wechat: { type: "string", description: "Reusable WeChat answer, supplied explicitly by the user." },
          internshipDuration: { type: "string", description: "Reusable internship-duration answer." },
          positionKeywords: {
            type: "string",
            description: "Reusable position keywords when an exact role is unavailable.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args, exec) {
          ...
          if (sessionId === undefined || sessionId.length === 0) {
            return { status: "invalid_request" as const, message: "candidate_profile_session_required" };
          }
          try {
            return {
              status: "ok" as const,
              preview: candidateProfile.preview(
                {
                  ...(typeof args.preferredCity === "string" ? { preferredCity: args.preferredCity } : {}),
                  ...(typeof args.arrivalTime === "string" ? { arrivalTime: args.arrivalTime } : {}),
                  ...(typeof args.wechat === "string" ? { wechat: args.wechat } : {}),
                  ...(typeof args.internshipDuration === "string"
                    ? { internshipDuration: args.internshipDuration }
                    : {}),
                  ...(typeof args.positionKeywords === "string"
                    ? { positionKeywords: args.positionKeywords }
                    : {}),
                },
                sessionId,
    ...
    ctx.tools.register(
      defineTool({
        name: "boss_watch_candidate_profile_apply",
        description:
          "Persist one exact short-lived local candidate preference preview after explicit confirmation. Does not inspect, fill, upload, submit, or contact an external site.",
        parameters: {
          previewToken: {
            type: "string",
            required: true,
            description: "Exact token returned by boss_watch_candidate_profile_preview.",
          },
          confirmed: {
            type: "boolean",
            required: true,
            description: "Must be true only after the user confirms the listed reusable fields.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args, exec) {
          ...
          try {
            return {
              status: "ok" as const,
              profile: candidateProfile.apply(
                args.previewToken,
                args.confirmed,
                sessionId,
              ) as unknown as JsonValue,
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Read local job-search readiness, source options, checkpoints, BOSS search guard state, and recommended next tools. Local read-only; the guard check reads Controller memory only and never refreshes a source, opens a page, applies, or writes Feishu.",
        parameters: {},
        output: {
          schema: {
            type: "object",
        ...
        async execute() {
          ...
          try {
            return {
              status: "ok" as const,
              overview: (await workspaceOverview.inspect()) as unknown as JsonValue,
            };
          } catch (error: unknown) {
      ...
      defineTool({
        ...
        async execute() {
          if (browser === undefined) {
            return {
              status: "environment_interrupted" as const,
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Preview a bounded BOSS keyword/city search. Does not open a page, capture a JD, send, apply, or write Feishu.",
        parameters: {
        ...
        async execute(args) {
          if (bossJobSearch === undefined)
            return { status: "source_unavailable" as const, message: "boss_search_unavailable" };
          try {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Run one confirmed bounded BOSS search preview. Opens generated search/detail pages serially and stops on login, verification, risk, or browser handoff.",
        parameters: {
          previewToken: { type: "string", required: true },
          confirmed: {
            type: "boolean",
            required: true,
            description: "Must be true only after the exact preview is confirmed.",
          },
        },
        output: {
          schema: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Preview a user-selected local Tencent CSV/XLSX export. Local read only; never contacts Tencent Docs or writes JobLead facts.",
        parameters: {
          sourceRef: {
            type: "string",
            required: true,
            description: "Configured source reference for the exported table.",
          },
          fileName: {
            type: "string",
            required: true,
            description: "File name inside the configured local import directory; never an arbitrary path.",
          },
          sheetName: {
            type: "string",
            description: "XLSX worksheet name. Required when the workbook has multiple sheets.",
          },
          columnMapping: {
            type: "json",
            description: "Optional explicit standard field to source column mapping.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          if (leadImport === undefined)
            return { status: "source_unavailable" as const, message: "lead_import_not_configured" };
          try {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Apply a previously previewed local Tencent CSV/XLSX import after explicit user confirmation. Writes only local SQLite facts.",
        parameters: {
          previewToken: {
            type: "string",
            required: true,
            description: "Short-lived token returned by boss_watch_lead_import_preview.",
          },
          confirmation: {
            type: "string",
            required: true,
            description: "Explicit confirmation of the displayed source, worksheet and import counts.",
          },
        },
        output: {
          schema: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Preview rows the user copied from a visible Tencent Docs table. Reads the local system clipboard only; never contacts Tencent Docs or writes JobLead facts.",
        parameters: {
          sourceRef: {
            type: "string",
            required: true,
            description: "Stable user-provided reference for the Tencent table/view.",
          },
          columnMapping: {
            type: "json",
            description: "Optional explicit standard field to source column mapping.",
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Apply a previously previewed clipboard snapshot after explicit user confirmation. Writes only local SQLite facts and rejects if the clipboard changed.",
        parameters: {
          previewToken: {
            type: "string",
            required: true,
            description: "Short-lived token returned by boss_watch_lead_clipboard_preview.",
          },
          confirmation: {
            type: "string",
            required: true,
            description: "Explicit confirmation of the displayed source and import counts.",
          },
        },
        output: {
          schema: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Preview structured job rows extracted from a user-provided screenshot. Validates only; never contacts Tencent Docs or writes SQLite.",
        parameters: {
          sourceRef: {
            type: "string",
            required: true,
            description: "Stable reference for the visible source/table.",
          },
          screenshotRef: {
            type: "string",
            required: true,
            description: "Temporary local attachment reference; remote URLs are not accepted.",
          },
          screenshotHash: {
            type: "string",
            description:
              "Optional SHA-256 assertion. The host recomputes it from the durable DSH attachment.",
          },
          sourceKind: {
            type: "string",
            enum: ["gankinterview_campus", "tencent_smart_sheet", "boss_visible", "company_career_site"],
          },
          ...
          columnMapping: {
            type: "json",
            description: "Optional canonical field to recognized header mapping.",
          },
          rows: {
            type: "json",
            required: true,
            description: "Structured rows from the vision model; each row needs company and role.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          if (visualImport === undefined)
            return { status: "source_unavailable" as const, message: "visual_lead_import_not_configured" };
          const rows = normalizeVisualRows(args.rows);
          if (rows === undefined)
            return { status: "invalid_request" as const, message: "visual_rows_required" };
          const headers = normalizeStringArray(args.headers);
          if (args.headers !== undefined && headers === undefined)
            return { status: "invalid_request" as const, message: "visual_headers_invalid" };
          try {
            const columnMapping = isRecord(args.columnMapping)
              ? normalizeImportColumnMapping(args.columnMapping)
              : undefined;
            const preview = await visualImport.preview({
              ...
              ...(typeof args.sourceKind === "string"
                ? { sourceKind: args.sourceKind as LeadSourceKind }
                : {}),
              ...(typeof args.sheetName === "string" ? { sheetName: args.sheetName } : {}),
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Apply a previously previewed screenshot result after explicit confirmation of source, row counts, and low-confidence warnings. Writes only local SQLite facts.",
        parameters: {
          previewToken: {
            type: "string",
            required: true,
            description: "Short-lived token returned by boss_watch_lead_visual_preview.",
          },
          confirmation: {
            type: "string",
            required: true,
            description:
              "Explicit confirmation of the displayed source, accepted rows, rejected rows and low-confidence rows.",
          },
        },
        output: {
          schema: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Read the local application tracker with current progress summaries. Read-only; refreshes from SQLite on every call.",
        parameters: {},
        output: {
          schema: {
            type: "object",
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Read a local application progress overview. Read-only; never changes status or visits an external site.",
        parameters: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Read the current local follow-up inbox and merge each reminder with the latest SQLite application timeline. Read-only; never contacts a recruiter or visits an external site.",
        parameters: {
        ...
        async execute(args) {
          if (followUpStore === undefined) {
            return {
              ...
              dueCount: 0,
              upcomingCount: 0,
              message: "follow_up_store_unavailable",
            };
          }
          const asOf =
            typeof args.asOf === "string" && args.asOf.trim().length > 0
              ? args.asOf
          ...
          try {
                ...
                return {
                  ...
                  ...(application === undefined
                    ...
                    : {
                        ...
                        ...(application.proposedStatus === undefined
                          ? {}
                          : { proposedStatus: application.proposedStatus }),
                      }),
                };
              }),
            );
            ...
            return {
              status: followUpInputError(error)
                ? ("invalid_request" as const)
                : ("source_unavailable" as const),
              asOf,
              items: [],
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Schedule a local follow-up reminder for an existing application. Local SQLite write only; does not send a message, change BOSS state, or write Feishu.",
        parameters: {
          applicationId: { type: "string", required: true, description: "Local application id." },
          dueAt: {
            type: "string",
            required: true,
            description: "ISO timestamp when the reminder should become due.",
          },
          reason: {
            type: "string",
            required: true,
          ...
          note: {
            type: "string",
            description: "Short local note, without credentials or full recruiter messages.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          if (followUpStore === undefined)
            return { status: "source_unavailable" as const, message: "follow_up_store_unavailable" };
          ...
          try {
            ...
            if (application === undefined)
              return { status: "not_found" as const, message: "application_not_found" };
            if (!isFollowUpReason(args.reason))
              return { status: "invalid_request" as const, message: "invalid_follow_up_reason" };
            if (typeof args.dueAt !== "string")
              return { status: "invalid_request" as const, message: "invalid_follow_up_due_at" };
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Mark one local follow-up reminder complete. Local SQLite write only; this does not claim that an external action succeeded.",
        parameters: {
          followUpId: {
            type: "string",
            required: true,
            description: "Follow-up id returned by boss_watch_follow_up_list or schedule.",
          },
        },
        output: {
          schema: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Search the configured GankInterview campus source and save a local JobLead snapshot. External read only; does not validate a JD or apply.",
        parameters: {
          page: { type: "integer" },
          limit: { type: "integer" },
          keyword: { type: "string" },
        ...
        async execute(args) {
          if (leadSource === undefined) {
            return {
              ...
              message: "gankinterview_not_configured",
            };
          }
          try {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Read locally persisted JobLead snapshots. Read-only; does not contact a source or open an external page.",
        parameters: {},
        output: {
          schema: {
            type: "object",
        ...
        async execute() {
          if (leadStore === undefined)
            return {
              status: "source_unavailable" as const,
              leads: [],
              count: 0,
              message: "job_lead_store_unavailable",
            };
          try {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Read one locally persisted JobLead and its source/verification fields. Read-only; does not navigate.",
        parameters: {
        ...
        async execute(args) {
          if (leadStore === undefined)
            return { status: "source_unavailable" as const, message: "job_lead_store_unavailable" };
          try {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Read append-only local source observations for JobLead snapshots. Read-only; never contacts GankInterview or Tencent Docs, opens a page, or changes a lead.",
        parameters: {
          ...
          since: {
            type: "string",
            description: "Optional ISO timestamp; includes observations at or after this time.",
          },
          includeUnchanged: {
            type: "boolean",
            description: "Include unchanged refreshes; default false returns only new/changed observations.",
          },
        },
        output: {
          schema: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Read recent local source snapshot status. Read-only; never refreshes GankInterview, Tencent Docs, BOSS, or an ATS.",
        parameters: {
          sourceKind: {
            type: "string",
            enum: ["gankinterview_campus", "tencent_smart_sheet", "boss_visible", "company_career_site"],
        ...
        async execute(args) {
          ...
          try {
            ...
            return {
              status: "ok" as const,
              snapshots: snapshots as unknown as JsonValue,
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Confirm that the current lead's saved candidate link is the reviewed HTTPS official/ATS URL. Local SQLite write only; accepts no arbitrary URL and does not navigate or apply.",
        parameters: {
          leadId: {
            type: "string",
            required: true,
            description: "Local JobLead id returned by boss_watch_lead_get.",
          },
          contentHash: {
            type: "string",
            required: true,
            description: "Exact current lead content hash shown to the user.",
          },
        },
        output: {
          schema: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Record explicit human confirmation that the already URL-verified page matches this company, role, and current JD. Local SQLite write only; does not navigate, fill, send, or apply.",
        parameters: {
          leadId: {
            type: "string",
            required: true,
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Create a local ordered application plan from JD-verified or human-confirmed leads. Local state only; does not open a page, fill a form, send, or apply.",
        parameters: {
          leadIds: {
            type: "array",
            items: { type: "string" },
            required: true,
            description: "Lead ids in the exact user-confirmed order.",
          },
        ...
        async execute(args) {
          if (batchStore === undefined)
            return { status: "source_unavailable" as const, message: "batch_store_unavailable" };
          if (
            !Array.isArray(args.leadIds) ||
            !args.leadIds.every((leadId): leadId is string => typeof leadId === "string")
          ) {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Read the latest local batch application plan, item states, failure reason and handoff checkpoint. Read-only.",
        parameters: {
          batchId: {
            type: "string",
            required: true,
            description: "Local batch id returned by boss_watch_apply_batch_prepare.",
          },
        },
        output: {
          schema: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Resume a paused local batch after the user has handled a platform handoff. Clears the stale checkpoint and returns the item to awaiting_gate_b; never retries or submits an external action.",
        parameters: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Preview one local PDF, DOCX, Markdown or TXT resume from the controlled resume directory. Reads bytes only to hash them; never returns resume content or writes SQLite.",
        parameters: {
          fileName: {
            type: "string",
            required: true,
            description:
              "File name inside the configured local resume directory; absolute paths and traversal are rejected.",
          },
          displayName: {
            type: "string",
            description: "Optional local display name for this resume version.",
          },
          supersedesResumeVersionId: {
            type: "string",
            description: "Optional existing resume version that this version supersedes.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          if (resumeImport === undefined)
            return { status: "source_unavailable" as const, message: "resume_import_unavailable" };
          try {
            return {
              status: "ok" as const,
              preview: (await resumeImport.preview({
                ...
                ...(typeof args.supersedesResumeVersionId === "string"
                  ? { supersedesResumeVersionId: args.supersedesResumeVersionId }
                  : {}),
              })) as unknown as JsonValue,
            };
          } catch (error: unknown) {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Apply one resume import preview after explicit confirmation. Rechecks the file hash and writes only version metadata plus a content-addressed local artifact.",
        parameters: {
          previewToken: {
            type: "string",
            required: true,
            description: "Short-lived token returned by boss_watch_resume_import_preview.",
          },
          confirmed: {
            type: "boolean",
            required: true,
            description:
              "Must be true after the user confirms the file name, hash, size and version relationship.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          ...
          if (args.confirmed !== true)
            return { status: "invalid_request" as const, message: "resume_import_confirmation_required" };
          try {
            const result = await resumeImport.apply(args.previewToken);
            return {
              status: "ok" as const,
              resumeVersion: result.resumeVersion as unknown as JsonValue,
              reused: result.reused,
            };
          } catch (error: unknown) {
            return resumeImportError(error);
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "List local resume version metadata. Read-only; never returns resume content or absolute paths.",
        parameters: {
          limit: {
            type: "integer",
            description: "Maximum versions to return, from 1 to 100. Defaults to 20.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          if (resumeStore === undefined)
            return {
              ...
              versions: [],
              count: 0,
              message: "resume_store_unavailable",
            };
          try {
            const versions = resumeStore.list(numberField(args.limit, "limit"));
            return {
              status: "ok" as const,
              versions: versions as unknown as JsonValue,
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Read one local resume version metadata record. Read-only; never returns resume content or absolute paths.",
        parameters: {
          resumeVersionId: {
            type: "string",
            required: true,
            description: "Resume version id returned by boss_watch_resume_list or import apply.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          if (resumeStore === undefined)
            return { status: "source_unavailable" as const, message: "resume_store_unavailable" };
          try {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Preview an official application page only after one exact Gate A confirmation and verified source binding. Read-only and local: never opens the page, reads the resume, fills fields, sends, or submits.",
        parameters: {
          leadId: {
            type: "string",
            required: true,
          ...
          gateAId: {
            ...
            required: true,
            description:
              "Exact Gate A id returned by boss_watch_gate_a_confirm for the bound application and resume snapshot.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          if (applicationPreview === undefined)
            return { status: "source_unavailable" as const, message: "application_preview_unavailable" };
          try {
            return {
              status: "ok" as const,
              preview: applicationPreview.preview({
                leadId: args.leadId,
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Match one registered local resume against one locally captured complete JD with local-evidence-match-v3. Returns only hashes, normalized technology/capability labels, eligibility and location-preference constraints, a privacy-bounded resume summary, score, gaps and risks; never returns raw resume/JD text or calls an external model.",
        parameters: {
          applicationId: {
            type: "string",
            required: true,
            description: "Application id whose full JD was already captured locally.",
          },
          resumeVersionId: {
            type: "string",
            required: true,
            description:
              "Existing local resume version id returned by boss_watch_resume_list or resume import apply.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          if (resumeMatching === undefined)
            return { status: "source_unavailable" as const, message: "resume_match_unavailable" };
          try {
            return {
              status: "ok" as const,
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "List persisted local resume match results, optionally for one application. Read-only; returns privacy-bounded summaries, evidence labels and hashes, never resume or JD raw text.",
        parameters: {
        ...
        async execute(args) {
          if (resumeMatchStore === undefined)
            return {
              status: "source_unavailable" as const,
              matches: [],
              count: 0,
              message: "resume_match_store_unavailable",
            };
          try {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Persist the user's explicit Gate A decision to proceed with materials for one exact local match snapshot. Local-only: does not open a page, fill or submit a form, send a message, or write Feishu.",
        parameters: {
          matchId: {
            type: "string",
            required: true,
            description:
              "Exact match id returned by boss_watch_resume_match or boss_watch_resume_match_list.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          if (gateA === undefined)
            return { status: "source_unavailable" as const, message: "gate_a_unavailable" };
          try {
            return {
              status: "ok" as const,
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Fill the already-open current ATS page in one local operation after the user explicitly says to fill the current page. Internally binds the current DSH session, exact Gate A, JD, resume hash, official origin, form hash and a short-lived content-hashed plan; fills deterministic text/dropdown fields and uploads the bound resume, then stops before next-step navigation, consent and final submit.",
        parameters: {
          leadId: {
            type: "string",
            required: true,
            description: "Verified local JobLead id for the already-open ATS page.",
          },
          gateAId: {
            type: "string",
            required: true,
            description: "Exact Gate A id fixing the application, JD and resume version.",
          },
          authorization: {
            type: "string",
            required: true,
            enum: ["fill_current_page"],
            description:
              "Set only when the user directly instructed Boss Watch in this conversation to fill the current ATS page.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args, exec) {
          if (applicationFormPreview === undefined) {
            return { status: "source_unavailable" as const, message: "application_form_autofill_unavailable" };
          }
          const sessionId = exec.agent?.id === undefined ? undefined : String(exec.agent.id);
          if (sessionId === undefined || sessionId.length === 0) {
            return { status: "invalid_request" as const, message: "application_form_session_required" };
          }
          if (args.authorization !== "fill_current_page") {
            return {
              status: "invalid_request" as const,
              message: "application_form_autofill_authorization_required",
            };
          }
          try {
            const outcome = await applicationFormPreview.autofill({
              leadId: args.leadId,
              gateAId: args.gateAId,
              sessionId,
            ...
            if (outcome.status === "filled") {
              return { status: "ok" as const, result: outcome as unknown as JsonValue };
            }
            if (outcome.status === "handoff_required") {
              return { status: "handoff_required" as const, handoff: outcome as unknown as JsonValue };
            }
            return { status: "conflict" as const, message: outcome.reason };
          } catch (error: unknown) {
            return applicationFormAutofillError(error);
          }
        },
      }),
    ),
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Inspect an already-opened official/ATS form and build a privacy-bounded batch plan fixed to one exact Gate A resume snapshot. The plan may include safe dropdown choices and a hash-bound latest resume upload; values stay local and this call never writes DOM or submits.",
        parameters: {
          leadId: {
            type: "string",
            required: true,
            description:
              "Verified local JobLead id whose stored officialApplyUrl identifies the allowed page origin.",
          },
          gateAId: {
            type: "string",
            required: true,
            description: "Exact Gate A id that fixes the application, JD and resume version.",
          },
        },
        output: {
          schema: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Apply one exact, short-lived local ATS batch plan after explicit user confirmation. Uses the preview-bound DSH session, Gate A resume, official origin and form hash; fills deterministic fields, safe dropdowns and the bound resume upload, then stops before consent or final submit.",
        parameters: {
          previewToken: {
            type: "string",
            required: true,
            description: "Exact token returned by boss_watch_application_form_preview.",
          },
          confirmed: {
            type: "boolean",
            required: true,
            description: "Must be true only after the user confirms this exact local prefill plan.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args, exec) {
          if (applicationFormPreview === undefined) {
            return { status: "source_unavailable" as const, message: "application_form_fill_unavailable" };
          }
          const sessionId = exec.agent?.id === undefined ? undefined : String(exec.agent.id);
          if (sessionId === undefined || sessionId.length === 0) {
            return { status: "invalid_request" as const, message: "application_form_session_required" };
          ...
          try {
            const outcome = await applicationFormPreview.apply({
            ...
            return { status: "conflict" as const, result: outcome as unknown as JsonValue };
          } catch (error: unknown) {
            return applicationFormFillError(error);
          }
        },
      }),
    ),
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Compare two locally captured JD revisions for one application. Read-only; never opens a page, calls a model, or writes facts.",
        parameters: {
          applicationId: {
            type: "string",
            required: true,
            description: "Local application id with at least two captured JD revisions.",
          },
          fromContentHash: {
            type: "string",
            description: "Optional SHA-256 hash of the older locally captured revision.",
          },
          toContentHash: {
            type: "string",
            description: "Optional SHA-256 hash of the newer locally captured revision.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          if (jobDiff === undefined)
            return { status: "source_unavailable" as const, message: "jd_diff_unavailable" };
          try {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Create a local low-frequency JD watch from an already captured BOSS application. Does not poll immediately or open a page.",
        parameters: {
          applicationId: {
            type: "string",
            required: true,
            description: "Existing local application id with a captured BOSS JD.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          if (jobWatch === undefined)
            return { status: "source_unavailable" as const, message: "job_watch_store_unavailable" };
          try {
        ...
        async execute() {
          if (jobWatch === undefined)
            return {
              ...
              watches: [],
              count: 0,
              message: "job_watch_store_unavailable",
            };
          try {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Poll one due local JD watch through the fixed stored BOSS URL. Applies interval, daily budget, and human handoff guards.",
        parameters: {
          watchId: {
            type: "string",
            required: true,
            description: "Local watch id returned by boss_watch_watch_create.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          ...
          try {
            return {
              status: "ok" as const,
              result: (await jobWatch.poll(args.watchId)) as unknown as JsonValue,
            };
          } catch (error: unknown) {
            return jobWatchError(error);
          }
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Run one explicit bounded pass over due local JD watches. Never starts a background loop; stops on handoff, transient failure, shared budget, or cancellation.",
        parameters: {
          limit: {
            type: "integer",
            description: "Maximum due watches in this pass. Defaults to 5 and cannot exceed 5.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args, exec) {
          if (jobWatchScheduler === undefined)
            return { status: "source_unavailable" as const, message: "job_watch_scheduler_unavailable" };
          if (
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Resume a paused local JD watch after the user handles login, verification, risk, or adapter handoff. Does not poll automatically.",
        parameters: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "List locally captured job descriptions. Read-only; never visits or applies to an external site.",
        parameters: {},
        output: {
          schema: {
            type: "object",
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Build a read-only Feishu Bitable field preview from local facts. Never writes to Feishu and always requires separate approval.",
        parameters: {
          applicationId: { type: "string", required: true, description: "Local application id." },
        },
        output: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Resolve a Feishu Base link and preview its table, view, fields, and automatic mapping. Read-only; never writes Feishu.",
        parameters: {
          url: {
            type: "string",
            required: true,
            description: "Feishu Base or Wiki URL supplied by the user.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          if (feishuProjection === undefined)
            return { status: "source_unavailable" as const, message: "feishu_projection_unavailable" };
          try {
            return {
              status: "ok" as const,
              preview: (await feishuProjection.targetPreview(args.url)) as unknown as JsonValue,
            };
          } catch (error: unknown) {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Persist a previously previewed Feishu target and field mapping locally after explicit confirmation. Does not write Feishu records.",
        parameters: {
          previewToken: {
            type: "string",
            required: true,
            description: "Short-lived token returned by boss_watch_feishu_target_preview.",
          },
          confirmed: {
            type: "boolean",
            required: true,
            description: "Must be true after the user confirms the target and mapping.",
          },
        },
        output: {
          schema: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Compare local applications with a confirmed Feishu target and preview create/update/unchanged/conflict changes. Read-only.",
        parameters: {
          targetId: {
            type: "string",
            required: true,
            description: "Local Feishu target id returned by target confirmation.",
          },
          applicationIds: {
            type: "array",
            items: { type: "string" },
            required: true,
            description: "Local application ids to project, in user-confirmed order.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          ...
          try {
            return {
              status: "ok" as const,
              preview: (await feishuProjection.syncPreview(
                args.targetId,
                args.applicationIds,
              )) as unknown as JsonValue,
            };
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Apply one exact Feishu sync preview after explicit confirmation. Writes only the previewed records and fields.",
        parameters: {
          previewToken: {
            type: "string",
            required: true,
            description: "Short-lived token returned by boss_watch_feishu_sync_preview.",
          },
          confirmed: {
            type: "boolean",
            required: true,
            description: "Must be true after the user confirms the exact create/update counts and fields.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          ...
          try {
            return {
              status: "ok" as const,
              result: (await feishuProjection.syncApply(
                args.previewToken,
                args.confirmed,
              )) as unknown as JsonValue,
            };
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Open and capture one exact BOSS job returned by boss_watch_discover_jobs. Read-only navigation; never sends or applies.",
        parameters: {
          discoveryId: {
            type: "string",
            required: true,
            description: "Short-lived id returned by boss_watch_discover_jobs.",
          },
          externalJobId: {
            type: "string",
            required: true,
            description: "Exact external job id returned in the selected discovery result.",
          },
        },
        output: {
          schema: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Check the local BossHunter browser runtime and supported BOSS job tab. Read-only; never clicks, navigates, or sends.",
        parameters: {},
        output: {
          schema: {
            type: "object",
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Capture the currently open unique BOSS job detail page through the local Controller. It never clicks, navigates, sends, or applies.",
        parameters: {},
        output: {
          schema: {
            type: "object",
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Capture the latest visible recruiter message from the unique selected BOSS conversation. Read-only; never replies, clicks, or sends.",
        parameters: {
          applicationId: {
            type: "string",
            required: true,
            description: "Existing local application id that this conversation belongs to.",
          },
        },
        output: {
          schema: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Classify pasted recruiting text or a staged .eml/.txt file locally. Returns hashes and a status proposal without writing SQLite or Feishu.",
        parameters: {
          ...
          content: {
            type: "string",
            description: "Pasted recruiting evidence. Mutually exclusive with stagedFileName.",
          },
          stagedFileName: {
            type: "string",
            description: "Controlled staged .eml/.txt file name returned by the DSH import button.",
          },
          sourceHash: {
            type: "string",
            description: "SHA-256 returned by the DSH import button; required with stagedFileName.",
          },
          declaredOutcome: {
            type: "string",
            enum: ["interview", "rejected", "offer", "needs_review"],
            description:
              "Only for a user-explicit manual_update; never infer or fill this from page content.",
          },
          observedAt: {
            type: "string",
            description: "Optional ISO timestamp for when the signal was observed.",
          },
        },
        output: {
          schema: {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Record exactly one previewed progress signal after explicit confirmation. Appends local evidence and, when supported, a status proposal; never writes Feishu or performs an external action.",
        parameters: {
          previewToken: {
            type: "string",
            required: true,
            description: "Short-lived token returned by boss_watch_progress_signal_preview.",
          },
          confirmed: {
            type: "boolean",
            required: true,
            description:
              "Must be true after the user confirms application, source, hash and proposed outcome.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          ...
          try {
            return {
              status: "ok" as const,
              result: (await progressSignalClient.apply(
                args.previewToken,
                args.confirmed,
              )) as unknown as JsonValue,
            };
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Preview one status that the user says they personally observed for an existing local application. Local read-only preview; does not log in, query an ATS, send, submit, or write Feishu.",
        parameters: {
          applicationId: {
            type: "string",
            required: true,
            description: "Exact existing local application id.",
          },
          status: {
            type: "string",
            required: true,
          ...
          occurredAt: {
            type: "string",
            description: "Optional ISO timestamp for when the user observed the status.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          if (applicationStatus === undefined)
            return {
              status: "source_unavailable" as const,
              message: "application_status_client_unavailable",
            };
          try {
            return {
              status: "ok" as const,
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Append exactly one user-confirmed local application status from a short-lived preview. Does not perform the external action or write Feishu.",
        parameters: {
          previewToken: {
            type: "string",
            required: true,
            description: "Short-lived token returned by boss_watch_application_status_preview.",
          },
          confirmed: {
            type: "boolean",
            required: true,
            description: "Must be true only after the user confirms the exact application, status and time.",
          },
        },
        output: {
          schema: {
        ...
        async execute(args) {
          ...
          try {
            return {
              status: "ok" as const,
              result: (await applicationStatus.apply(
                args.previewToken,
                args.confirmed,
              )) as unknown as JsonValue,
            };
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Preview a manually entered interview note for one existing application. Does not write SQLite until the matching apply call is confirmed.",
        parameters: {
          applicationId: { type: "string", required: true, description: "Existing local application id." },
          interviewId: {
            type: "string",
            required: true,
            description: "User-defined stable interview identifier.",
          },
          stage: {
            type: "string",
            required: true,
        ...
        async execute(args) {
          if (interviewNoteClient === undefined)
            return { status: "source_unavailable" as const, message: "interview_note_client_unavailable" };
          try {
    ...
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Apply exactly one interview-note preview after the user confirms the application, interview id, stage and hash. Writes only the previewed local evidence.",
        parameters: {
          previewToken: {
            type: "string",
            required: true,
            description: "Short-lived token returned by boss_watch_interview_note_preview.",
          },
          confirmed: {
            type: "boolean",
            required: true,
        ...
        async execute(args) {
          ...
          try {
            return {
              status: "ok" as const,
              result: (await interviewNoteClient.apply(
                args.previewToken,
                args.confirmed,
              )) as unknown as JsonValue,
            };
...
function stableError(error: unknown): string {
  if (error instanceof Error && error.message === "source_unavailable")
    return "boss_watch_source_unavailable";
  if (error instanceof Error && error.message === "source_corrupt_event") return "boss_watch_source_corrupt";
  if (error instanceof Error && error.message === "gankinterview_unauthorized")
    return "gankinterview_unauthorized";
  if (error instanceof Error && error.message === "gankinterview_rate_limited")
    return "gankinterview_rate_limited";
  if (error instanceof Error && error.message === "gankinterview_unavailable")
    return "gankinterview_unavailable";
  if (error instanceof Error && error.message === "gankinterview_invalid_response")
    return "gankinterview_invalid_response";
...
function interviewNoteError(error: unknown): {
  ...
  return {
    ...
    message: message === "controller_unavailable" ? message : "interview_note_unavailable",
  };
}
...
function progressSignalError(error: unknown): {
  ...
  if (
    message === "application_not_found" ||
    message === "progress_signal_preview_not_found" ||
    message === "progress_signal_file_not_found"
  ) {
    return { status: "not_found", message };
  }
  ...
  return {
    status: "source_unavailable",
    message: message === "controller_unavailable" ? message : "progress_signal_unavailable",
  };
}

function leadImportError(error: unknown): {
  status: "invalid_request" | "not_found" | "conflict" | "source_unavailable";
  message: string;
  details?: string[];
} {
  ...
  const conflict = new Set([
    ...
    "clipboard_changed_since_preview",
  ]);
  ...
  return {
    status,
    message,
    ...(error instanceof LeadImportError && error.details !== undefined
...
function visualLeadImportError(error: unknown): {
  status: "invalid_request" | "not_found" | "conflict" | "source_unavailable";
  message: string;
} {
...
function feishuProjectionError(error: unknown): {
  status: "invalid_request" | "not_found" | "conflict" | "source_unavailable";
  message: string;
} {
...
function jobWatchError(error: unknown): {
  status: "invalid_request" | "not_found" | "conflict" | "source_unavailable";
  message: string;
} {
...
function jobDiffError(error: unknown): {
  status: "invalid_request" | "not_found" | "source_unavailable";
  message: string;
} {
  ...
  const invalid = new Set([
    "invalid_application_id",
    "invalid_jd_diff_from_content_hash",
    "invalid_jd_diff_to_content_hash",
  ]);
  if (notFound.has(message)) return { status: "not_found", message };
  if (invalid.has(message)) return { status: "invalid_request", message };
  return { status: "source_unavailable", message };
...
function applicationPreviewError(error: unknown): {
  status: "invalid_request" | "not_found" | "conflict" | "source_unavailable";
  message: string;
} {
  const message = error instanceof Error ? error.message : "application_preview_failed";
  if (
    message === "apply_lead_not_found" ||
    message === "apply_resume_not_found" ||
    message === "apply_gate_a_not_found"
  )
    return { status: "not_found", message };
  if (
    new Set(["apply_lead_not_verified", "apply_gate_a_binding_missing", "apply_resume_snapshot_stale"]).has(
      message,
    )
  )
    return { status: "conflict", message };
  if (
    new Set([
      "apply_official_url_missing",
      "apply_official_url_invalid",
      "invalid_gate_a_id",
      "invalid_lead_id",
    ]).has(message)
  ) {
...
type ApplicationFormToolError = {
  status: "invalid_request" | "not_found" | "conflict" | "source_unavailable";
  message: string;
};

function applicationFormPreviewError(error: unknown): ApplicationFormToolError {
  return applicationFormToolError(error, "application_form_preview_failed");
}

function applicationFormAutofillError(error: unknown): ApplicationFormToolError {
  return applicationFormToolError(error, "application_form_autofill_failed");
}

function applicationFormFillError(error: unknown): ApplicationFormToolError {
  return applicationFormToolError(error, "application_form_fill_failed");
}

function applicationFormToolError(error: unknown, fallback: string): ApplicationFormToolError {
  ...
  const conflict = new Set([
    ...
    "ats_autofill_resume_identity_mismatch",
    "ats_autofill_profile_hash_mismatch",
  ...
  const invalid = new Set([
    ...
    "application_form_fill_confirmation_required",
    "application_form_autofill_authorization_required",
  ]);
  const unavailable = new Set([
    ...
    "ats_autofill_profile_store_closed",
  ]);
  if (notFound.has(message)) return { status: "not_found", message };
  if (conflict.has(message)) return { status: "conflict", message };
  if (invalid.has(message)) return { status: "invalid_request", message };
  if (unavailable.has(message)) return { status: "source_unavailable", message };
  return { status: "source_unavailable", message: fallback };
}

function candidateProfileError(error: unknown): {
  status: "invalid_request" | "not_found" | "conflict" | "source_unavailable";
  message: string;
} {
  const message = error instanceof Error ? error.message : "candidate_profile_failed";
  if (message === "candidate_profile_preview_not_found" || message === "candidate_profile_not_configured") {
    return { status: "not_found", message };
  }
  if (
    new Set([
      "candidate_profile_session_mismatch",
      "candidate_profile_preview_expired",
      "candidate_profile_preview_consumed",
      "candidate_profile_hash_mismatch",
    ]).has(message)
  )
    return { status: "conflict", message };
  if (
    message === "candidate_profile_empty" ||
    message === "candidate_profile_confirmation_required" ||
    message === "candidate_profile_session_required" ||
    message === "invalid_session_id" ||
    message === "invalid_candidate_profile_preview_token" ||
    message.startsWith("invalid_candidate_profile_")
  )
    return { status: "invalid_request", message };
  if (message === "candidate_profile_store_closed") return { status: "source_unavailable", message };
  return { status: "source_unavailable", message: "candidate_profile_failed" };
}

function resumeImportError(error: unknown): {
  status: "invalid_request" | "not_found" | "conflict" | "source_unavailable";
  message: string;
} {
  ...
  const notFound = new Set([
    ...
    "resume_supersedes_not_found",
  ]);
  const conflict = new Set([
    "resume_preview_stale",
    "resume_import_in_progress",
    "resume_artifact_hash_conflict",
  ]);
  const invalid = new Set([
...
function resumeStoreError(error: unknown): {
  status: "invalid_request" | "source_unavailable";
  message: string;
} {
...
function gateAError(error: unknown): {
  status: "invalid_request" | "not_found" | "source_unavailable";
  message: string;
} {
...
function applicationStatusError(error: unknown): {
  status: "invalid_request" | "not_found" | "conflict" | "source_unavailable";
  message: string;
} {
  const message = error instanceof Error ? error.message : "application_status_failed";
  if (message === "application_not_found" || message === "application_status_preview_not_found")
    return { status: "not_found", message };
  if (message === "confirmation_required") return { status: "conflict", message };
  if (
    new Set([
      "invalid_application_status",
      "invalid_application_status_timestamp",
      "invalid_application_status_apply",
  ...
  return {
    status: "source_unavailable",
    message:
      message === "controller_unavailable" || message === "controller_restart_required"
        ? message
...
function batchError(error: unknown): {
  ...
  if (!(error instanceof Error))
    return { status: "source_unavailable", message: "boss_watch_batch_read_failed" };
  if (
    error.message === "lead_not_found" ||
    error.message === "batch_not_found" ||
    error.message === "batch_item_not_found"
  ) {
    return { status: "not_found", message: error.message };
  }
  if (
    error.message === "lead_not_verified" ||
    error.message === "lead_content_changed" ||
    error.message === "batch_not_paused" ||
    error.message === "checkpoint_not_resumable" ||
    error.message === "invalid_item_transition"
  ) {
...
function leadConfirmationError(error: unknown): {
  status: "invalid_request" | "not_found" | "conflict" | "source_unavailable";
  message: string;
} {
  if (!(error instanceof Error))
    return { status: "source_unavailable", message: "boss_watch_lead_confirmation_failed" };
...
function leadObservationError(error: unknown): {
  status: "invalid_request" | "source_unavailable";
  observations: JobLeadObservation[];
  count: number;
  newCount: number;
  changedCount: number;
  ...
  const invalid = new Set([
    "invalid_lead_observation_limit",
    "invalid_lead_source_kind",
    "invalid_lead_observation_since",
  ]);
  return {
...
function countObservationKind(
  observations: readonly JobLeadObservation[],
  kind: JobLeadObservation["changeKind"],
): number {
...
function isFollowUpReason(value: unknown): value is FollowUpReason {
  return (
    value === "application_status" || value === "no_response" || value === "interview" || value === "manual"
  );
}
...
function followUpNextAction(reason: FollowUpReason): string {
  switch (reason) {
    case "application_status":
      return "核对本地投递状态和最近事件";
    case "no_response":
      return "人工判断是否需要跟进，不自动发送消息";
    case "interview":
      return "核对面试时间并准备面试，不自动接受邀请";
    case "manual":
      return "按本地备注处理";
  }
}
...
function materializeBatch(
  batch: import("./application-batch.js").BatchApplicationRun,
): import("./application-batch.js").BatchApplicationRun {
