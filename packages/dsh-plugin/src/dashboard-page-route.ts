...
<title>Boss Watch · 求职看板</title>
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .metric { min-width: 0; padding: 11px 13px; border: 1px solid var(--border); border-radius: var(--radius); background: var(--surface); }
...
<body>
  ...
  <div class="app">
    <header class="app-header">
      ...
      <div class="header-actions"><span class="runtime" id="runtime-status" role="status"><span class="dot warn"></span>读取中</span><button class="button" id="refresh" type="button">刷新</button><a class="button primary" href="/" target="_blank" rel="noopener noreferrer">打开 DSH</a></div>
    </header>
    <main class="main" id="main-content" tabindex="-1">
      ...
      <section class="hidden" id="board-content" aria-label="岗位池">
        <div class="metrics" id="metrics" aria-label="岗位概览"></div>
        <div class="workspace">
          <section class="panel" aria-labelledby="jobs-title">
            ...
            <div class="empty hidden" id="jobs-empty"><strong id="jobs-empty-title">还没有岗位</strong><span id="jobs-empty-hint">从 DSH 获取岗位或导入岗位表后会显示在这里。</span></div>
  ...
  <script>
    (() => {
      ...
      const labels = {
        ...
        worth: { recommended: '值得投', review: '可复核', not_recommended: '暂不投', pending: '待评估' },
      ...
      function displayProfile(candidate) {
        ...
        if (candidate.jdStatus === 'complete' && match && match.matchLevel !== 'insufficient_evidence') {
          if (match.matchLevel === 'strong') { worth = 'recommended'; reason = match.score + ' 分，高匹配，建议优先核对并投递'; }
          else if (match.matchLevel === 'moderate') { worth = 'review'; reason = match.score + ' 分，中等匹配，建议先复核缺口'; }
          else { worth = 'not_recommended'; reason = match.score + ' 分，当前匹配较弱'; }
        }
      ...
      async function useDraft(candidate) { const request = { type: 'boss-watch:dashboard-draft', delivery: 'draft_only', autoSubmit: false, draft: buildDraft(candidate) }; if (state.embedded && window.parent !== window) { window.parent.postMessage(request, window.location.origin); return; } try { await navigator.clipboard.writeText(request.draft); showToast('投递准备草稿已复制，请发送前复核'); } catch (_) { showToast('剪贴板不可用，请在 DSH 中打开求职看板'); } }
      ...
      function renderMetrics() {
        const rows = boardPool(); const now = Date.now(); const cutoff = now + 7 * 86400000;
        const recommended = rows.filter((candidate) => displayProfile(candidate).worth === 'recommended').length;
        const deadlines = rows.filter((candidate) => { const time = Date.parse(candidate.deadline || ''); return Number.isFinite(time) && time >= now && time <= cutoff; }).length;
        const progressed = rows.filter((candidate) => Boolean(candidate.confirmedStatus || candidate.proposedStatus || candidate.gateA)).length;
        $('metrics').replaceChildren(metric('岗位总数', rows.length), metric('值得投', recommended), metric('7 天内截止', deadlines), metric('已有进度', progressed));
      }
      ...
      function renderJobs() {
        ...
        setText($('jobs-meta'), rows.length + ' 条结果 · 第 ' + page + ' / ' + totalPages + ' 页'); $('jobs-empty').classList.toggle('hidden', rows.length !== 0); setText($('jobs-empty-title'), hasFilters() ? '没有符合条件的岗位' : '还没有岗位'); setText($('jobs-empty-hint'), hasFilters() ? '调整筛选或清空条件后重试。' : '从 DSH 获取岗位或导入岗位表后会显示在这里。'); $('filter-reset').classList.toggle('hidden', !hasFilters()); $('pagination').classList.toggle('hidden', rows.length === 0); $('page-prev').disabled = page <= 1; $('page-next').disabled = page >= totalPages; setText($('page-status'), '第 ' + page + ' / ' + totalPages + ' 页');
        $('jobs-body').replaceChildren(...pageRows.map((candidate) => {
          const profile = displayProfile(candidate); const row = document.createElement('tr'); row.tabIndex = 0; row.setAttribute('aria-selected', String(state.selected === candidate.candidateId)); row.setAttribute('aria-label', candidate.company + ' ' + text(candidate.role, '待选择岗位'));
          const identity = cell(); const title = document.createElement('div'); title.className = 'job-main'; title.textContent = text(candidate.role, '待选择具体岗位'); const sub = document.createElement('div'); sub.className = 'job-sub'; sub.textContent = candidate.company + (candidate.city ? ' · ' + candidate.city : ''); identity.append(title, sub);
          ...
          const action = cell(); const actions = document.createElement('div'); actions.className = 'cell-actions'; const url = safeUrl(candidate.officialApplyUrl); if (url) actions.append(verifiedLink(candidate, url)); actions.append(draftButton(candidate)); action.append(actions);
      ...
      function resetPageAndSelection() { state.page = 1; state.pageNeedsNormalization = false; state.selected = null; syncUrl('replace'); renderJobs(); renderDetail(); }
      function render(snapshot) { state.snapshot = snapshot; $('loading-panel').classList.add('hidden'); $('error-panel').classList.add('hidden'); $('board-content').classList.remove('hidden'); renderMetrics(); renderJobs(); renderDetail(); restoreControls(); const guard = snapshot.overview.bossSearchGuard; const runtime = $('runtime-status'); runtime.replaceChildren(); const dot = document.createElement('span'); dot.className = 'dot ' + (guard.guarded ? 'warn' : ''); runtime.append(dot, document.createTextNode(guard.guarded ? '岗位采集暂缓' : '岗位信息已更新')); }
      async function load() { const refresh = $('refresh'); refresh.disabled = true; $('error-panel').classList.add('hidden'); if (!state.snapshot) $('loading-panel').classList.remove('hidden'); try { const response = await fetch(API, { method: 'GET', headers: { accept: 'application/json' }, credentials: 'same-origin', cache: 'no-store' }); const payload = await response.json(); if (!response.ok || payload.status !== 'ok' || payload.readOnly !== true || !Array.isArray(payload.candidates)) throw new Error(payload?.error?.code || 'dashboard_unavailable'); render(payload); } catch (error) { $('loading-panel').classList.add('hidden'); $('board-content').classList.add('hidden'); $('error-panel').classList.remove('hidden'); setText($('error-message'), error instanceof Error ? error.message : 'unknown_error'); const runtime = $('runtime-status'); runtime.replaceChildren(); const dot = document.createElement('span'); dot.className = 'dot error'; runtime.append(dot, document.createTextNode('读取失败')); } finally { refresh.disabled = false; } }
      ...
      $('filter-reset').addEventListener('click', () => { state.query = ''; state.match = 'all'; state.companyCategory = 'all'; state.roleDirection = 'all'; state.sort = 'match'; restoreControls(); resetPageAndSelection(); });
      ...
      window.addEventListener('popstate', () => { Object.assign(state, parseQuery()); restoreControls(); if (state.snapshot) { renderJobs(); renderDetail(); } });
