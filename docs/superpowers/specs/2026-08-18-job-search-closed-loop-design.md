...
- 将 Gank/Tencent 来源候选与 BOSS 完整 JD 并列展示在统一候选面板；BOSS 支持固定关键词/城市的短批次搜索，最多 2 页/5 个岗位，跨页按岗位 ID 去重并串行捕获详情。
...
- 不把微信内推群或 Tailscale 手机链路当作已实现的后台读取；未来只通过用户主动复制/导出接入 `MessageSource`，不读取微信数据库或自动发送消息。
