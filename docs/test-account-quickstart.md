...
3. 在 `http://127.0.0.1:3080/` 配置可用模型，新建对话并输入：
...
4. 登录完成后重新检查同源唯一表单。表单预览会给出字段分类和本地批量预填计划；当前 Agent 侧填表审批入口未接入，字段写入仍由用户处理。
5. 用户手动上传简历、填写敏感/未知字段、勾选隐私项并点击最终提交。DSH 不把“预填完成”说成“已投递”。
6. 用户告诉 DSH“已投递/完成笔试/收到面试”等事实，DSH 先生成状态预览；确认后追加本地时间线，再生成飞书同步预览，用户再次确认才写入多维表格。

今晚验收时，准备一条已核验官网岗位和一份本地 PDF/DOCX 简历即可。验收重点是：匹配是否可解释、登录 handoff 后能否重新预览、本地预填计划是否受 session/form hash 约束、敏感项是否仍留给人工、提交后状态是否能进入本地看板和 Feishu 预览。
